...
exports.createOrder = async (req, res) => {
    ...
    try {
        ...
        const { delivery_address, address, contact, payment_method, items, total } = req.body;
        ...
        // Validate contact field (must be exactly 11 digits)
        if (!contact || !/^[0-9]{11}$/.test(contact)) {
            return res.status(400).json({
                status: 'error',
                message: 'Contact must contain only numbers and be exactly 11 digits'
            });
        }
