...
async function getAllIssues() {
  ...
  while (!isLast) {
    ...
    const path = `/rest/api/2/search?jql=${encodeURIComponent(jql)}&startAt=${startAt}&maxResults=${maxResults}&fields=key,summary,status`;
    ...
    try {
      ...
      // Try falling back to jql endpoint
      console.log('📝 Trying alternative endpoint...\n');
      return await getAllIssuesViaJql();
    }
  }
...
// Alternative method using JQL endpoint
async function getAllIssuesViaJql() {
  console.log('📥 Fetching all issues via JQL endpoint...\n');
  
  let allIssues = [];
  
  try {
    const jql = `project = "${PROJECT_KEY}"`;
    const path = `/rest/api/2/search?jql=${encodeURIComponent(jql)}&maxResults=100&fields=key,summary,status`;
    ...
    console.error('Error in alternative fetch:', error.message);
  }
...
async function transitionIssue(issueKey, targetStatus) {
  try {
    ...
    if (!transition) {
      console.log(`   ⚠️  No transition available to "${targetStatus}"`);
      return false;
    }
...
async function updateIssueStatuses() {
  ...
  try {
    ...
    for (let i = 0; i < allIssues.length; i++) {
      ...
      process.stdout.write(`[${i + 1}/${allIssues.length}] ${issue.key}: ${issue.summary.substring(0, 40)}... → ${newStatus}... `);
    ...
    console.log(`✅ Successfully Updated: ${successfulUpdates}/${allIssues.length}`);
    console.log(`❌ Failed Updates: ${failedUpdates}/${allIssues.length}`);
    console.log(`\n📈 Final Status Counts:`);
