              ...
              <div id="productsContainer" className="row">
                {isLoadingProducts ? (
                  ...
                  filteredProducts.map((product) => (
                    <div className="col-6 col-md-6 col-lg-4 col-xl-3 col-xxl-3 mb-3" key={product.id}>
                      <ProductCard
                        product={product}
                        onAddToCart={(prod) => {
                          if (!user) {
                          ...
                          if (Number(prod.stock) <= 0) {
                            showAlert('This item is currently out of stock.', 'error', 'Unavailable');
                            return;
                          }
                          addToCart({
                            id: prod.id,
                            name: prod.name,
                            price: prod.price,
                            iconClass: getProductIconClass(prod.iconCategory || prod.category)
                          });
                        }}
                        onAddToWishlist={(prod) => {
                          if (!user) {
                            setLoginNotice('You need to login to add to wishlist.');
                            showAlert('Please login to add to wishlist.', 'error', 'Login required');
                            return;
                          }
                          showAlert(`${prod.name} added to wishlist!`, 'success');
                        }}
                        isLoggedIn={!!user}
                        getProductIconClass={getProductIconClass}
                      />
                    </div>
                  ))
                )}
