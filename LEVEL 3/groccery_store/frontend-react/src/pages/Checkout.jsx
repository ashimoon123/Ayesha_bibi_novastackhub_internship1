...
export default function Checkout() {
  ...
  const canSubmit = useMemo(() => {
    return (
      ...
      form.city &&
      form.postalCode &&
      /^[0-9]{1,6}$/.test(form.postalCode) &&
      form.contact &&
      /^[0-9]{11}$/.test(form.contact) &&
      cart.length > 0
    );
  }, [form, cart]);
            ...
            <div className="card mb-4">
              ...
              <div className="card-body">
                ...
                <div className="row">
                  ...
                  <div className="col-md-6 mb-3">
                    <label className="form-label fw-bold">Postal Code (6 digits, numbers only)</label>
                    <input
                      type="text"
                      className="form-control"
                      name="postalCode"
                      ...
                      onChange={handleChange}
                      placeholder="e.g., 75270"
                      maxLength="6"
                      required
                    />
                    {form.postalCode && !/^[0-9]{1,6}$/.test(form.postalCode) && (
                      <small className="text-danger">Postal code must contain only numbers and be max 6 digits</small>
                    )}
                  </div>
                </div>
                <div className="mb-3">
                  <label className="form-label fw-bold">Contact Number (11 digits, numbers only)</label>
                  <input
                    type="tel"
                    className="form-control"
                    name="contact"
                    ...
                    required
                  />
                  {form.contact && !/^[0-9]{11}$/.test(form.contact) && (
                    <small className="text-danger">Contact must contain only numbers and be 11 digits</small>
                  )}
                </div>
              </div>
            </div>
